...
async function aiRoute(request, env) {
  ...
  let aiText;
  try {
    aiText = extractAiText(await runAnalysisModel(env, model, prompt, verbose));
    ...
    return json({ error: "ai_request_failed", message: safeErrorMessage(error) }, 502);
  }
...
<body>
  ...
  <main>
    ...
    <section class="panel" id="aiPanel">
      <div class="row" style="justify-content:space-between"><h3>AI 分析</h3><button id="aiBtn" class="admin-only">AI 分析</button></div>
      <div id="aiResult" class="notice muted">管理员登录后可基于当前筛选域名的筛选数据，分析异常盗用、自动探测或正常波动。</div>
  ...
  <script>
    ...
    function st(id, text) { const el = $(id); if (el) el.textContent = text; }
    let chart;
    let trafficChart;
    ...
    $("aiBtn").onclick = () => { const host = $("hostInput").value.trim(); const projectName = $("projectInput").value.trim(); if (!session.aiAvailable) { st("aiResult", "当前 Pages 项目未绑定 Workers AI，请根据仓库 README 在 Functions 设置中添加 AI 绑定后重新部署。"); return; } if (!accountCache.length) { st("aiResult", NO_ACCOUNT_MESSAGE); return; } if ($("accountSelect").value === "all") { st("aiResult", "请先选择一个具体账号，再进行 AI 盗用风险分析。"); return; } if (!host) { st("aiResult", "请先输入主机名，再基于筛选后的数据进行 AI 盗用风险分析。"); return; } if (!projectName) { st("aiResult", "AI 分析必须输入服务名 / 项目名参数。"); return; } openAiConfirm(host); };
